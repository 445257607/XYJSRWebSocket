...
NSString *XYJSRURLOrigin(NSURL *url)
{
...
extern BOOL XYJSRURLRequiresSSL(NSURL *url)
{
    NSString *scheme = url.scheme.lowercaseString;
    return ([scheme isEqualToString:@"wss"] || [scheme isEqualToString:@"https"]);
}

extern NSString *_Nullable XYJSRBasicAuthorizationHeaderFromURL(NSURL *url)
{
    NSData *data = [[NSString stringWithFormat:@"%@:%@", url.user, url.password] dataUsingEncoding:NSUTF8StringEncoding];
    return [NSString stringWithFormat:@"Basic %@", XYJSRBase64EncodedStringFromData(data)];
}

extern NSString *_Nullable XYJSRStreamNetworkServiceTypeFromURLRequest(NSURLRequest *request)
{
