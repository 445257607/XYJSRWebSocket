...
static void XYJSRMaskBytesManual(uint8_t *bytes, size_t length, uint8_t *maskKey) {
 ...
 */
static uint8x32_t XYJSRShiftVector(uint8x32_t vector, size_t by) {
...
void XYJSRMaskBytesSIMD(uint8_t *bytes, size_t length, uint8_t *maskKey) {
    ...
    if (alignmentBytes > length || (length - alignmentBytes) < sizeof(uint8x32_t)) {
        XYJSRMaskBytesManual(bytes, length, maskKey);
        return;
    }
    ...
    maskVector = XYJSRShiftVector(maskVector, alignmentBytes);

    XYJSRMaskBytesManual(bytes, alignmentBytes, maskKey);
    ...
    XYJSRMaskBytesManual(bytes + manualStartOffset, manualLength, (uint8_t *) &maskVector);
}
