...
@interface XYJSRProxyConnect() <NSStreamDelegate>
...
@implementation XYJSRProxyConnect
{
    XYJSRProxyConnectCompletion _completion;
...
-(instancetype)initWithURL:(NSURL *)url
{
    ...
    _url = url;
    _connectionRequiresSSL = XYJSRURLRequiresSSL(url);
...
- (void)dealloc
{
    ...
    [self.inputStream removeFromRunLoop:[NSRunLoop XYJSR_networkRunLoop] forMode:NSDefaultRunLoopMode];
...
- (void)_didConnect
{
    XYJSRDebugLog(@"_didConnect, return streams");
    if (_connectionRequiresSSL) {
        if (_httpProxyHost) {
            // Must set the real peer name before turning on SSL
            XYJSRDebugLog(@"proxy set peer name to real host %@", self.url.host);
    ...
    [inputStream removeFromRunLoop:[NSRunLoop XYJSR_networkRunLoop] forMode:NSDefaultRunLoopMode];
...
{
    XYJSRDebugLog(@"_failWithError, return error");
    if (!error) {
        error = XYJSRHTTPErrorWithCodeDescription(500, 2132,@"Proxy Error");
    }
    ...
    [self.inputStream removeFromRunLoop:[NSRunLoop XYJSR_networkRunLoop]
...
- (void)_configureProxy
{
    ...
    if (proxies.count == 0) {
        XYJSRDebugLog(@"configureProxy no proxies");
...
- (void)_readProxySettingWithType:(NSString *)proxyType settings:(NSDictionary *)settings
{
    ...
    if (_httpProxyHost) {
        XYJSRDebugLog(@"Using http proxy %@:%u", _httpProxyHost, _httpProxyPort);
    } else if (_socksProxyHost) {
        XYJSRDebugLog(@"Using socks proxy %@:%u", _socksProxyHost, _socksProxyPort);
    } else {
        XYJSRDebugLog(@"configureProxy no proxies");
    }
}
...
{
    XYJSRDebugLog(@"SRWebSocket fetchPAC:%@", PACurl);
...
- (void)_openConnection
{
    ...
    [self.inputStream scheduleInRunLoop:[NSRunLoop XYJSR_networkRunLoop]
                                forMode:NSDefaultRunLoopMode];
...
- (void)_initializeStreams
{
    ...
    XYJSRDebugLog(@"ProxyConnect connect stream to %@:%u", host, port);
    ...
    if (_socksProxyHost) {
        XYJSRDebugLog(@"ProxyConnect set sock property stream to %@:%u user %@ password %@", _socksProxyHost, _socksProxyPort, _socksProxyUsername, _socksProxyPassword);
...
{
    XYJSRDebugLog(@"stream handleEvent %u", eventCode);
    switch (eventCode) {
...
- (void)_proxyDidConnect
{
    ...
    XYJSRDebugLog(@"Proxy sending %@", connectRequestStr);
...
{
    NSMutableData *buf = [NSMutableData dataWithCapacity:XYJSRDefaultBufferSize()];
    uint8_t *buffer = buf.mutableBytes;
    NSInteger length = [_inputStream read:buffer maxLength:XYJSRDefaultBufferSize()];
...
- (void)_proxyHTTPHeadersDidFinish
{
    ...
    if (responseCode >= 299) {
        XYJSRDebugLog(@"Connect to Proxy Request failed with response code %d", responseCode);
    ...
    XYJSRDebugLog(@"proxy connect return %d, call socket connect", responseCode);
    [self _didConnect];
}
...
- (void)_writeData:(NSData *)data
{
    ...
    dispatch_async(_writeQueue, ^{
        ...
        while (![outStream hasSpaceAvailable]) {
            ...
            timeout -= 100;
            if (timeout < 0) {
                NSError *error = XYJSRHTTPErrorWithCodeDescription(408, 2132, @"Proxy timeout");
