...
#import "XYJSRDelegateController.h"
#import "XYJSRIOConsumer.h"
#import "XYJSRIOConsumerPool.h"
#import "XYJSRHash.h"
#import "XYJSRURLUtilities.h"
#import "XYJSRError.h"
#import "NSURLRequest+XYJSRWebSocket.h"
#import "NSRunLoop+XYJSRWebSocket.h"
#import "XYJSRProxyConnect.h"
#import "XYJSRSecurityPolicy.h"
#import "XYJSRHTTPConnectMessage.h"
#import "XYJSRRandom.h"
#import "XYJSRLog.h"
#import "XYJSRMutex.h"
#import "XYJSRSIMDHelpers.h"
#import "NSURLRequest+XYJSRWebSocketPrivate.h"
#import "NSRunLoop+XYJSRWebSocketPrivate.h"
#import "XYJSRConstants.h"
...
{
    import_NSURLRequest_XYJSRWebSocket();
    import_NSRunLoop_XYJSRWebSocket();
}
...
static NSString *const XYJSRWebSocketAppendToSecKeyString = @"258EAFA5-E914-47DA-95CA-C5AB0DC85B11";

static inline int32_t validate_dispatch_data_partial_string(NSData *data);

static uint8_t const XYJSRWebSocketProtocolVersion = 13;

NSString *const XYJSRWebSocketErrorDomain = @"SRWebSocketErrorDomain";
NSString *const XYJSRHTTPResponseErrorKey = @"HTTPResponseStatusCode";

@interface XYJSRWebSocket ()  <NSStreamDelegate>

@property (atomic, assign, readwrite) XYJSRReadyState readyState;
...
@property (nonatomic, strong, readonly) XYJSRDelegateController *delegateController;

@end

@implementation XYJSRWebSocket {
    XYJSRMutex _kvoLock;
    OSSpinLock _propertyLock;

    dispatch_queue_t _workQueue;
    NSMutableArray<XYJSRIOConsumer *> *_consumers;
    ...
    XYJSRSecurityPolicy *_securityPolicy;
    ...
    __strong XYJSRWebSocket *_selfRetain;

    NSArray<NSString *> *_requestedProtocols;
    XYJSRIOConsumerPool *_consumerPool;

    // proxy support
    XYJSRProxyConnect *_proxyConnect;
}
...
- (instancetype)initWithURLRequest:(NSURLRequest *)request protocols:(NSArray<NSString *> *)protocols securityPolicy:(XYJSRSecurityPolicy *)securityPolicy
{
    self = [super init];
    if (!self) return self;
    ...
    _requestRequiresSSL = XYJSRURLRequiresSSL(_url);

    _readyState = XYJSR_CONNECTING;

    _propertyLock = OS_SPINLOCK_INIT;
    _kvoLock = XYJSRMutexInitRecursive();
    _workQueue = dispatch_queue_create(NULL, DISPATCH_QUEUE_SERIAL);

    // Going to set a specific on the queue so we can validate we're on the work queue
    dispatch_queue_set_specific(_workQueue, (__bridge void *)self, (__bridge void *)(_workQueue), NULL);

    _delegateController = [[XYJSRDelegateController alloc] init];
    ...
    _consumerPool = [[XYJSRIOConsumerPool alloc] init];
...
{
    XYJSRSecurityPolicy *securityPolicy;
    BOOL certificateChainValidationEnabled = !allowsUntrustedSSLCertificates;

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated"

    securityPolicy = [[XYJSRSecurityPolicy alloc] initWithCertificateChainValidationEnabled:certificateChainValidationEnabled];
...
- (instancetype)initWithURLRequest:(NSURLRequest *)request securityPolicy:(XYJSRSecurityPolicy *)securityPolicy
{
...
- (instancetype)initWithURL:(NSURL *)url securityPolicy:(XYJSRSecurityPolicy *)securityPolicy
{
...
- (void)setReadyState:(XYJSRReadyState)readyState
{
    @try {
        XYJSRMutexLock(_kvoLock);
    ...
    @finally {
        XYJSRMutexUnlock(_kvoLock);
    }
}

- (XYJSRReadyState)readyState
{
    XYJSRReadyState state = 0;
    OSSpinLockLock(&_propertyLock);
    state = _readyState;
    OSSpinLockUnlock(&_propertyLock);
    return state;
...
{
    assert(_url);
    NSAssert(self.readyState == XYJSR_CONNECTING, @"Cannot call -(void)open on SRWebSocket more than once.");
    ...
    if (_urlRequest.timeoutInterval > 0) {
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^{
            if (self.readyState == XYJSR_CONNECTING) {
                NSError *error = XYJSRErrorWithDomainCodeDescription(NSURLErrorDomain, NSURLErrorTimedOut, @"Timed out connecting to server.");
                [self _failWithError:error];
            }
        });
    }

    _proxyConnect = [[XYJSRProxyConnect alloc] initWithURL:_url];
...
- (void)_connectionDoneWithError:(NSError *)error readStream:(NSInputStream *)readStream writeStream:(NSOutputStream *)writeStream
{
    if (error != nil) {
        ...
        if (!_scheduledRunloops.count) {
            [self scheduleInRunLoop:[NSRunLoop XYJSR_networkRunLoop] forMode:NSDefaultRunLoopMode];
        }
...
- (BOOL)_checkHandshake:(CFHTTPMessageRef)httpMessage;
{
    ...
    NSString *concattedString = [_secKey stringByAppendingString:XYJSRWebSocketAppendToSecKeyString];
    NSData *hashedString = XYJSRSHA1HashFromString(concattedString);
    NSString *expectedAccept = XYJSRBase64EncodedStringFromData(hashedString);
...
- (void)_HTTPHeadersDidFinish;
{
    ...
    if (responseCode >= 400) {
        XYJSRDebugLog(@"Request failed with response code %d", responseCode);
        NSError *error = XYJSRHTTPErrorWithCodeDescription(responseCode, 2132,
    ...
    if(![self _checkHandshake:_receivedHTTPHeaders]) {
        NSError *error = XYJSRErrorWithCodeDescription(2133, @"Invalid Sec-WebSocket-Accept response.");
        [self _failWithError:error];
        return;
    }
    ...
    if (negotiatedProtocol) {
        ...
        if ([_requestedProtocols indexOfObject:negotiatedProtocol] == NSNotFound) {
            NSError *error = XYJSRErrorWithCodeDescription(2133, @"Server specified Sec-WebSocket-Protocol that wasn't requested.");
            [self _failWithError:error];
            return;
        }
    ...
    self.readyState = XYJSR_OPEN;

    if (!_didFail) {
        [self _readFrameNew];
    }

    [self.delegateController performDelegateBlock:^(id<XYJSRWebSocketDelegate>  _Nullable delegate, XYJSRDelegateAvailableMethods availableMethods) {
...
- (void)_readHTTPHeader;
{
    ...
    [self _readUntilHeaderCompleteWithCallback:^(XYJSRWebSocket *socket,  NSData *data) {
        CFHTTPMessageAppendBytes(_receivedHTTPHeaders, (const UInt8 *)data.bytes, data.length);

        if (CFHTTPMessageIsHeaderComplete(_receivedHTTPHeaders)) {
            XYJSRDebugLog(@"Finished reading headers %@", CFBridgingRelease(CFHTTPMessageCopyAllHeaderFields(_receivedHTTPHeaders)));
...
- (void)didConnect;
{
    XYJSRDebugLog(@"Connected");

    _secKey = XYJSRBase64EncodedStringFromData(XYJSRRandomData(16));
    assert([_secKey length] == 24);

    CFHTTPMessageRef message = XYJSRHTTPConnectMessageCreate(_urlRequest,
                                                          _secKey,
                                                          XYJSRWebSocketProtocolVersion,
                                                          self.requestCookies,
                                                          _requestedProtocols);
...
{
    if (_requestRequiresSSL) {
        XYJSRDebugLog(@"Setting up security for streams.");
        [_securityPolicy updateSecurityOptionsInStream:_inputStream];
        [_securityPolicy updateSecurityOptionsInStream:_outputStream];
    }

    NSString *networkServiceType = XYJSRStreamNetworkServiceTypeFromURLRequest(_urlRequest);
...
- (void)close;
{
    [self closeWithCode:XYJSRStatusCodeNormal reason:nil];
}
...
- (void)closeWithCode:(NSInteger)code reason:(NSString *)reason;
{
    assert(code);
    dispatch_async(_workQueue, ^{
        if (self.readyState == XYJSR_CLOSING || self.readyState == XYJSR_CLOSED) {
            return;
        }

        BOOL wasConnecting = self.readyState == XYJSR_CONNECTING;

        self.readyState = XYJSR_CLOSING;

        XYJSRDebugLog(@"Closing with code %d reason %@", code, reason);
...
#pragma unused (success)
        ...
        [self _sendFrameWithOpcode:XYJSROpCodeConnectionClose data:payload];
    });
}
...
- (void)_closeWithProtocolError:(NSString *)message;
{
    ...
    [self.delegateController performDelegateQueueBlock:^{
        [self closeWithCode:XYJSRStatusCodeProtocolError reason:message];
        dispatch_async(_workQueue, ^{
            [self closeConnection];
        });
    }];
...
{
    dispatch_async(_workQueue, ^{
        if (self.readyState != XYJSR_CLOSED) {
            _failed = YES;
            [self.delegateController performDelegateBlock:^(id<XYJSRWebSocketDelegate>  _Nullable delegate, XYJSRDelegateAvailableMethods availableMethods) {
            ...
            self.readyState = XYJSR_CLOSED;

            XYJSRDebugLog(@"Failing with error %@", error.localizedDescription);
...
{
    if (self.readyState != XYJSR_OPEN) {
        NSString *message = @"Invalid State: Cannot call `sendString:error:` until connection is open.";
        if (error) {
            *error = XYJSRErrorWithCodeDescription(2134, message);
        }
        XYJSRDebugLog(message);
        return NO;
    }

    string = [string copy];
    dispatch_async(_workQueue, ^{
        [self _sendFrameWithOpcode:XYJSROpCodeTextFrame data:[string dataUsingEncoding:NSUTF8StringEncoding]];
    });
    return YES;
}
...
- (BOOL)sendDataNoCopy:(nullable NSData *)data error:(NSError **)error
{
    ...
    dispatch_async(_workQueue, ^{
        if (data) {
            [self _sendFrameWithOpcode:XYJSROpCodeBinaryFrame data:data];
        } else {
            [self _sendFrameWithOpcode:XYJSROpCodeTextFrame data:nil];
        }
    });
    return YES;
}
...
- (BOOL)sendPing:(nullable NSData *)data error:(NSError **)error
{
    ...
    dispatch_async(_workQueue, ^{
        [self _sendFrameWithOpcode:XYJSROpCodePing data:data];
    });
    return YES;
}
...
- (void)_handlePingWithData:(nullable NSData *)data
{
    ...
    [self.delegateController performDelegateBlock:^(id<XYJSRWebSocketDelegate> _Nullable delegate, XYJSRDelegateAvailableMethods availableMethods) {
        ...
        dispatch_async(_workQueue, ^{
            [self _sendFrameWithOpcode:XYJSROpCodePong data:data];
        });
    }];
}
...
- (void)handleCloseWithData:(NSData *)data;
{
    ...
    XYJSRDebugLog(@"Received close frame");
    ...
    if (dataSize == 1) {
        ...
        _closeCode = XYJSRStatusNoStatusReceived;
    }
...
{
    [self assertOnWorkQueue];
    XYJSRDebugLog(@"Trying to disconnect");
    _closeWhenFinishedWriting = YES;
    [self _pumpWriting];
}

- (void)_handleFrameWithData:(NSData *)frameData opCode:(XYJSROpCode)opcode
{
    // Check that the current data is valid UTF8

    BOOL isControlFrame = (opcode == XYJSROpCodePing || opcode == XYJSROpCodePong || opcode == XYJSROpCodeConnectionClose);
    if (isControlFrame) {
    ...
    switch (opcode) {
        case XYJSROpCodeTextFrame: {
            NSString *string = [[NSString alloc] initWithData:frameData encoding:NSUTF8StringEncoding];
            if (!string && frameData) {
                [self closeWithCode:XYJSRStatusCodeInvalidUTF8 reason:@"Text frames must be valid UTF-8."];
                dispatch_async(_workQueue, ^{
                    [self closeConnection];
                });
                return;
            }
            XYJSRDebugLog(@"Received text message.");
            [self.delegateController performDelegateBlock:^(id<XYJSRWebSocketDelegate>  _Nullable delegate, XYJSRDelegateAvailableMethods availableMethods) {
        ...
        case XYJSROpCodeBinaryFrame: {
            XYJSRDebugLog(@"Received data message.");
            [self.delegateController performDelegateBlock:^(id<XYJSRWebSocketDelegate>  _Nullable delegate, XYJSRDelegateAvailableMethods availableMethods) {
                if (availableMethods.didReceiveMessage) {
                    [delegate webSocket:self didReceiveMessage:frameData];
                }
            ...
            break;
        case XYJSROpCodeConnectionClose:
            [self handleCloseWithData:frameData];
            break;
        case XYJSROpCodePing:
            [self _handlePingWithData:frameData];
            break;
        case XYJSROpCodePong:
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    BOOL isControlFrame = (frame_header.opcode == XYJSROpCodePing || frame_header.opcode == XYJSROpCodePong || frame_header.opcode == XYJSROpCodeConnectionClose);
    ...
    if (frame_header.payload_length == 0) {
        ...
        [self _addConsumerWithDataLength:(size_t)frame_header.payload_length callback:^(XYJSRWebSocket *sself, NSData *newData) {
            if (isControlFrame) {
...
- (void)_readFrameContinue;
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(XYJSRWebSocket *sself, NSData *data) {
        ...
        BOOL isControlFrame = (receivedOpcode == XYJSROpCodePing || receivedOpcode == XYJSROpCodePong || receivedOpcode == XYJSROpCodeConnectionClose);
        ...
        if (extra_bytes_needed == 0) {
            ...
            [sself _addConsumerWithDataLength:extra_bytes_needed callback:^(XYJSRWebSocket *eself, NSData *edata) {
...
- (void)_pumpWriting;
{
    ...
    if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
        ...
        if (streamFailed) {
            ...
            NSError *error = streamError ? XYJSRErrorWithCodeDescriptionUnderlyingError(code, description, streamError) : XYJSRErrorWithCodeDescription(code, description);
            [self _failWithError:error];
            return;
        }
        ...
        if (_outputBufferOffset > XYJSRDefaultBufferSize() && _outputBufferOffset > dataLength / 2) {
...
- (void)_scheduleCleanup
{
    @synchronized(self) {
        ...
        [[NSRunLoop XYJSR_networkRunLoop] addTimer:timer forMode:NSDefaultRunLoopMode];
    }
}
...
- (BOOL)_innerPumpScanner {
    ...
    XYJSRIOConsumer *consumer = [_consumers objectAtIndex:0];
    ...
    if (consumer.readToCurrentFrame || foundSize) {
        ...
        if (_readBufferOffset > XYJSRDefaultBufferSize() && _readBufferOffset > readBufferSize / 2) {
        ...
        if (consumer.readToCurrentFrame) {
            ...
            if (_currentFrameOpcode == XYJSROpCodeTextFrame) {
                // Validate UTF8 stuff.
                size_t currentDataSize = _currentFrameData.length;
                if (_currentFrameOpcode == XYJSROpCodeTextFrame && currentDataSize > 0) {
...
- (void)_sendFrameWithOpcode:(XYJSROpCode)opCode data:(NSData *)data
{
    [self assertOnWorkQueue];
    ...
    if (!frameData) {
        [self closeWithCode:XYJSRStatusCodeMessageTooBig reason:@"Message too big"];
        return;
    }
    ...
    XYJSRMaskBytesSIMD(frameBufferPayloadPointer, payloadLength, maskKey);
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode
{
    ...
    if (_requestRequiresSSL && !_streamSecurityValidated &&
        ...
        if (!_streamSecurityValidated) {
            dispatch_async(_workQueue, ^{
                NSError *error = XYJSRErrorWithDomainCodeDescription(NSURLErrorDomain,
...
- (void)safeHandleEvent:(NSStreamEvent)eventCode stream:(NSStream *)aStream
{
    switch (eventCode) {
        case NSStreamEventOpenCompleted: {
            XYJSRDebugLog(@"NSStreamEventOpenCompleted %@", aStream);
            if (self.readyState >= XYJSR_CLOSING) {
                return;
            }
            assert(_readBuffer);

            if (!_requestRequiresSSL && self.readyState == XYJSR_CONNECTING && aStream == _inputStream) {
                [self didConnect];
            }
        ...
        case NSStreamEventErrorOccurred: {
            XYJSRDebugLog(@"NSStreamEventErrorOccurred %@ %@", aStream, [[aStream streamError] copy]);
        ...
        case NSStreamEventEndEncountered: {
            [self _pumpScanner];
            XYJSRDebugLog(@"NSStreamEventEndEncountered %@", aStream);
                        ...
                        [self.delegateController performDelegateBlock:^(id<XYJSRWebSocketDelegate>  _Nullable delegate, XYJSRDelegateAvailableMethods availableMethods) {
                            if (availableMethods.didCloseWithCode) {
                                [delegate webSocket:self
                                   didCloseWithCode:XYJSRStatusCodeGoingAway
        ...
        case NSStreamEventHasBytesAvailable: {
            XYJSRDebugLog(@"NSStreamEventHasBytesAvailable %@", aStream);
            uint8_t buffer[XYJSRDefaultBufferSize()];

            while (_inputStream.hasBytesAvailable) {
                NSInteger bytesRead = [_inputStream read:buffer maxLength:XYJSRDefaultBufferSize()];
                if (bytesRead > 0) {
                    dispatch_data_t data = dispatch_data_create(buffer, bytesRead, nil, DISPATCH_DATA_DESTRUCTOR_DEFAULT);
                    if (!data) {
                        NSError *error = XYJSRErrorWithCodeDescription(XYJSRStatusCodeMessageTooBig,
        ...
        case NSStreamEventHasSpaceAvailable: {
            XYJSRDebugLog(@"NSStreamEventHasSpaceAvailable %@", aStream);
            [self _pumpWriting];
            break;
        }

        case NSStreamEventNone:
            XYJSRDebugLog(@"(default)  %@", aStream);
            break;
    }
}
...
- (id<XYJSRWebSocketDelegate> _Nullable)delegate
{
    return self.delegateController.delegate;
}

- (void)setDelegate:(id<XYJSRWebSocketDelegate> _Nullable)delegate
{
